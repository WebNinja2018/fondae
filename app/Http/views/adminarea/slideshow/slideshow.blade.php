...
<?php
	$slideshowID=Input::get('slideshowID')?Input::get('slideshowID'):'';
...
<form role="form" name="frm_slideshow_list" method="post" action="{{ url('/') }}/adminarea/slideshow/index">
<input type="hidden" name="status" value=""  />
<input type="hidden" value="" name="slideshowID" />
...
<input type="hidden" name="redirects_to" value="{{Request::fullUrl()}}" />
    ...
    <section class="content-header top_header_content">
        <div class="addproperty_button">
            <div class="col-sm-12 col-md-12 col-xs-12">
               <div class="row">
                    <div class="">
                        <a href="javascript:fun_edit(0);" class="add_edit_btn"><i class="fa fa-plus"></i>&nbsp; Add New Slideshow</a>
						<a href="javascript:fun_changeOrder();" class="add_edit_btn"><i class="glyphicon glyphicon-align-justify"></i>&nbsp;&nbsp;Change Display Order</a>
                        <a class="add_edit_btn" href="#" id="advance_search" onclick="funToggleSearch('slideshow');"><i class="fa fa-search"></i>&nbsp; Search</a>
	...
	<section class="content-header top_header_content" id="searchSectionslideshow" @if(Session::get('searchSectionslideshow')!= 1) style=" display:none;" @endif> 
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12" role="main">
                            <ul class="pagination pagination-sm search_by_alphabetic">
                              <li><a href="javascript:fun_slideshowSearch('')">All</a></li>
                              @for($i=65;$i<=90;$i++)
                                  <li><a href="javascript:fun_slideshowSearch('{{ chr($i) }}')">{{ chr($i) }}</a></li>
                              @endfor
                            </ul>
                            <div class="form-group">
                                 <div class="col-sm-2 col-md-2 col-xs-2">
                                    <input type="text" name="srach_title" class="form-control" id="srach_title" value="{{ $srach_title }}" placeholder="Slideshow Title">
                                </div>
                                 <div class="col-sm-2 col-md-2 col-xs-2">
	...
	<section class="content-header top_header_content">
		<div class="col-xs-12 col-sm-12 col-md-12">
        	<div class="row">
                <!-- start slideshow management table-->
                  ...
                  <table id="example" class="table-bordered table-striped table-hover table">
                             ...
                             <th>Slideshow Title 
                                    <div class="pull-left">
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('slideshowTitle','ASC');" class="asc_desc_icon" >
                                                <span class="glyphicon glyphicon-triangle-top" aria-hidden="true"></span>
                                            </a>
                                        </div>
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('slideshowTitle','DESC');" class="asc_desc_icon">
                              ...
                              <th>Caption  
                                    <div class="pull-left">
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('caption1','ASC');" class="asc_desc_icon" >
                                                <span class="glyphicon glyphicon-triangle-top" aria-hidden="true"></span>
                                            </a>
                                        </div>
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('caption1','DESC');" class="asc_desc_icon">
                                                <span class="glyphicon glyphicon-triangle-bottom" aria-hidden="true" ></span>
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @if (($recordcount)>0)
                            @if(isset($recordcount))<?php $no_row=1; ?>
                                @foreach($qGetAllSlideshow as $GetAllSlideshow)
                                    <tr id="ID_{{ $GetAllSlideshow->slideshowID }}" @if($GetAllSlideshow->slideshowID==$slideshowID) class="bg-warning" @endif>
                                        <td>
                                            @if($GetAllSlideshow->slideshowID !=1)
                                                <input class="checkbox" name="checkUncheck[]"  id="checkAllAuto" value="{{ $GetAllSlideshow->slideshowID }}" type="checkbox" />
                                            @endif
                                        </td>
                                        <td>
                                            <a href="javascript:fun_edit({{ $GetAllSlideshow->slideshowID }});" title="Edit" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            <a href="javascript:fun_single_delete({{ $GetAllSlideshow->slideshowID }});" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        </td>
                                        <td>{{ $GetAllSlideshow->slideshowTitle }}</td>
                                        <td>{{ $GetAllSlideshow->caption1 }}</td>
                                        <td>{{ $GetAllSlideshow->displayOrder }}</td>
                                        <td>{{ date('m/d/Y',strtotime($GetAllSlideshow->created_at)) }}</td>
                                        <td>
                                            @if($GetAllSlideshow->slideshowID ==1)
                                            <span id="status_{{ $GetAllSlideshow->slideshowID }}">
                                                @if($GetAllSlideshow->isActive==1) Active @else Inactive @endif
                                            </span>
                                            @else
                                            <a href="javascript:fun_single_status({{ $GetAllSlideshow->slideshowID }});">
                                                <span id="status_{{ $GetAllSlideshow->slideshowID }}">
                                                    @if($GetAllSlideshow->isActive==1) Active @else Inactive @endif
                                                </span>
                                            </a>
                                        ...
                                        <td><a href="" data-toggle="modal" data-target="#myModal_{{ $GetAllSlideshow->slideshowID }}" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            @include('adminarea.slideshow.slideshow-view')
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
                ...
                <!-- end slideshow management table-->
                <div class="pagination_box" id="paggination">
                    <nav aria-label="Page navigation">
						<?php /*?>{!! $qGetAllSlideshow->render() !!}<?php */?>
                        @include('adminarea.pagination', ['paginator' => $qGetAllSlideshow])
                    </nav>
                </div>
              <!-- strat box area-->
             </div>
