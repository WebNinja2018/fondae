...
<script>
	$(function() {
		  $('.submitbuttonSave').click(function()
		  {
			  ...
			  $.ajax({
				type: "POST",
				url: "{{ url('/') }}/adminarea/links/saveorder",
				data: 'item='+h,
...
<form class="form-inline" role="form" name="frm_links_addedit" method="post" action="{{ url('/') }}/adminarea/links/saveorder">
...
<div class="right_side" role="main">
    <div class="center_container">
        <ul class="breadcrumb">
          ...
          <li class="active">Links Order</li>
        </ul>
    </div>
    <div class="panel row">
                            ...
                            <div id="sortable">
								...
								<?php $no_row=1; ?>
                                     @foreach($qGetAllLinks as $GetAlllinks)
                                      <div id="image_li_{{$GetAlllinks->linksID}}" class="innerproduct_order_col ui-sortable-handle" >
                                         {{ $GetAlllinks->name}}
                                      </div>
                                    <?php $no_row++; ?>
									@endforeach
