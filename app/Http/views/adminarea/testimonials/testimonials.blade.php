...
<?php
	$testimonialsID=Input::get('testimonialsID')?Input::get('testimonialsID'):'';
...
<form role="form" name="frm_testimonials_list" method="post" action="{{ url('/') }}/adminarea/testimonials/index">
<input type="hidden" name="status" value=""  />
<input type="hidden" value="" name="testimonialsID" />
...
<input type="hidden" name="redirects_to" value="{{Request::fullUrl()}}" />
    ...
    <section class="content-header top_header_content">
        <div class="addproperty_button">
            <div class="col-sm-12 col-md-12 col-xs-12">
               <div class="row">
                    <div class="">
                        <a href="javascript:fun_edit(0);" class="add_edit_btn"><i class="fa fa-plus"></i>&nbsp; Add New Testimonials</a>
                        <a href="javascript:fun_changeOrder();" class="add_edit_btn"><i class="glyphicon glyphicon-align-justify"></i>&nbsp;&nbsp;Change Display Order</a>
						<a class="add_edit_btn" href="#" id="advance_search" onclick="funToggleSearch('Testimonials');"><i class="fa fa-search"></i>&nbsp; Search</a>
                    </div>
               </div>
            </div>
        </div>
    </section>
	<section class="content-header top_header_content" id="searchSectionTestimonials" @if(Session::get('searchSectionTestimonials')!= 1) style=" display:none;" @endif> 
                   ...
                   <div class="row">
                       <div class="col-md-12 col-sm-12 col-xs-12" role="main">
                            <ul class="pagination pagination-sm search_by_alphabetic">
                              <li><a href="javascript:fun_testimonialsSearch('')">All</a></li>
                              @for($i=65;$i<=90;$i++)
                                  <li><a href="javascript:fun_testimonialsSearch('{{ chr($i) }}')">{{ chr($i) }}</a></li>
                              @endfor
                            </ul>
                            <div class="form-group">
                                 <div class="col-sm-2 col-md-2 col-xs-2">
                                     <input type="text" name="srach_name" class="form-control" id="srach_name" value="{{ $srach_name }}" placeholder="Testimonials Title">
                                 </div>
                                 <div class="col-sm-2 col-md-2 col-xs-2">
	...
	<section class="content-header top_header_content">
		<div class="col-xs-12 col-sm-12 col-md-12">
        	<div class="row">
                <!-- start testimonials management table-->
                  ...
                  <table id="example" class="table-bordered table-striped table-hover table">
                        <thead>
                             ...
                             <th>Client Name 
                                    <div class="pull-left">
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('clientName','ASC');" class="asc_desc_icon" >
                                                <span class="glyphicon glyphicon-triangle-top" aria-hidden="true"></span>
                                            </a>
                                        </div>
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('clientName','DESC');" class="asc_desc_icon">
                             ...
                             <th>Designation
                                    <div class="pull-left">
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('designation','ASC');" class="asc_desc_icon" >
                                                <span class="glyphicon glyphicon-triangle-top" aria-hidden="true"></span>
                                            </a>
                                        </div>
                                        <div class="col-md-12 col-xs-12 col-sm-12">
                                            <a href="javascript:;" onclick="fun_upDown('designation','DESC');" class="asc_desc_icon">
                                                <span class="glyphicon glyphicon-triangle-bottom" aria-hidden="true" ></span>
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @if (($recordcount)>0)
                            @if(isset($recordcount))<?php $no_row=1; ?>
                                @foreach($qGetAllTestimonials as $GetAllTestimonials)
                                    <tr id="ID_{{ $GetAllTestimonials->testimonialsID }}" @if($GetAllTestimonials->testimonialsID==$testimonialsID) class="bg-warning" @endif>
                                        <td>
                                            <input class="checkbox" name="checkUncheck[]"  id="checkAllAuto" value="{{ $GetAllTestimonials->testimonialsID }}" type="checkbox" />
                                        </td>
                                        <td>
                                            <a href="javascript:fun_edit({{ $GetAllTestimonials->testimonialsID }});" title="Edit" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            <a href="javascript:fun_single_delete({{ $GetAllTestimonials->testimonialsID }});" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        </td>
                                        <td>{{ $GetAllTestimonials->clientName }}</td>
                                        <td>{{ $GetAllTestimonials->designation }}</td>
                                        <td>{{ $GetAllTestimonials->displayOrder }}</td>
                                        <td>{{ date('m/d/Y',strtotime($GetAllTestimonials->created_at)) }}</td>
                                        <td>
                                            
                                            <a href="javascript:fun_single_status({{ $GetAllTestimonials->testimonialsID }});">
                                                <span id="status_{{ $GetAllTestimonials->testimonialsID }}">
                                                    @if($GetAllTestimonials->isActive==1) Active @else Inactive @endif
                                                </span>
                                            </a>
                                        </td>
                                        <td><a href="" data-toggle="modal" data-target="#myModal_{{ $GetAllTestimonials->testimonialsID }}" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            @include('adminarea.testimonials.testimonials-view')
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
                ...
                <!-- end testimonials management table-->
                <div class="pagination_box" id="paggination">
                    <nav aria-label="Page navigation">
						<?php /*?>{!! $qGetAllTestimonials->render() !!}<?php */?>
                        @include('adminarea.pagination', ['paginator' => $qGetAllTestimonials])
                    </nav>
                </div>
              <!-- strat box area-->
             </div>
