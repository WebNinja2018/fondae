...
<script>
	$(function() {
		  $('.submitbuttonSave').click(function()
		  {
			  ...
			  $.ajax({
				type: "POST",
				url: "{{ url('/') }}/adminarea/staff/saveorder",
				data: 'item='+h,
...
<form class="form-inline" role="form" name="frm_staff_addedit" method="post" action="{{ url('/') }}/adminarea/staff/saveorder">
...
<div class="right_side" role="main">
    <div class="center_container">
        <ul class="breadcrumb">
          ...
          <li class="active">staff Order</li>
        </ul>
    </div>
    <div class="panel row">
                            ...
                            <div id="sortable">
								...
								<?php $no_row=1; ?>
                                     @foreach($qGetAllStaff as $GetAllstaff)
                                      <div id="image_li_{{$GetAllstaff->staffID}}" class="innerproduct_order_col ui-sortable-handle" >
                                         {{ $GetAllstaff->firstname}} {{$GetAllstaff->lastname }}
                                      </div>
                                    <?php $no_row++; ?>
									@endforeach
                                 @else
