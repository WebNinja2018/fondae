...
<script>
	$(function() {
		  $('.submitbuttonSave').click(function()
		  {
			  ...
			  $.ajax({
				type: "POST",
				url: "{{ url('/') }}/adminarea/coupon/saveorder",
				data: 'item='+h,
...
<form class="form-inline" role="form" name="frm_coupon_addedit" method="post" action="{{ url('/') }}/adminarea/coupon/saveorder">
...
<div class="right_side" role="main">
    <div class="center_container">
        <ul class="breadcrumb">
          ...
          <li class="active">Coupon Order</li>
        </ul>
    </div>
    <div class="panel row">
                            ...
                            <div id="sortable">
								...
								<?php $no_row=1; ?>S
                                     @foreach($qGetAllCoupon as $GetAllcoupon)
                                      <div id="image_li_{{$GetAllcoupon->couponID}}" class="innerproduct_order_col ui-sortable-handle" >
                                         {{ $GetAllcoupon->couponName}}
                                      </div>
                                    <?php $no_row++; ?>
									@endforeach
                                 @else
