<script type="text/javascript">
	...
	{
		frmobj=window.document.frm_reward;
	...
	{
		var frmobj=window.document.frm_reward;
		frmobj.submit();
	}
	...
	function fun_rewardDetail(orderID)
	{
		var frmobj=window.document.frm_reward;
...
<form role="form" name="frm_reward" id="frm_reward" method="post" action="{{ url('/') }}/adminarea/order">
                ...
                <div class="table-responsive">
                  <table id="example" class="table-bordered table-striped table-hover table">
                        <thead>
							 ...
							 <th width="10%">Donation Number</th>
                             <th width="10%">Member Name</th>
                             <th width="10%">Reward Type</th>
                             <th width="10%">Reward Email</th>
                    		 <th width="10%">Created Date</th>
                             <th width="5%">View</th>
                        </thead>
                        <tbody>
                        <?php //dd($qGetProductRewardData);?>
                        @if (($prodcutRewardRecordCount)>0)
                            @if(isset($prodcutRewardRecordCount)) <?php $no_row=1;?>
                                @foreach($qGetProductRewardData as $GetAllOrder)
                                    <tr  id="ID_{{ $GetAllOrder->orderID }}" @if($GetAllOrder->orderID==$orderID) class="bg-warning" @endif>
                                        ...
                                        <td>@if($GetAllOrder->unpaidOption==1) Email @elseif($GetAllOrder->unpaidOption==2) Facebook @elseif($GetAllOrder->unpaidOption==3) Twitter @else Instagram @endif</td>
                                        <td>@if(strlen($GetAllOrder->orderEmail)>0){{ $GetAllOrder->orderEmail }}@else - @endif</td>
                                        <td>{{ date('m/d/Y',strtotime($GetAllOrder->created_at)) }}</td>
										<td align="center">
											<a class="glyphicon glyphicon-eye-open" href="" data-toggle="modal" data-target="#myModal_{{$GetAllOrder->orderID}}" onclick="fun_rewardDetail({{ $GetAllOrder->orderID}});"></a> &nbsp;&nbsp;
                             			</td>
                                    </tr>
                                    <?php $no_row++;?> 
                                @endforeach
