...
<script>
	$(function() {
		  $('.submitbuttonSave').click(function()
		  {
			  ...
			  $.ajax({
				type: "POST",
				url: "{{ url('/') }}/adminarea/faq/saveorder",
				data: 'item='+h,
...
<form class="form-inline" role="form" name="frm_faq_addedit" method="post" action="{{ url('/') }}/adminarea/faq/saveorder">
                            ...
                            <div id="sortable">
								...
								<?php $no_row=1; ?>
                                     @foreach($qGetAllFaq as $GetAllfaq)
                                      <div id="image_li_{{$GetAllfaq->faqID}}" class="innerproduct_order_col ui-sortable-handle" >
                                         {{ $GetAllfaq->question}}
                                      </div>
                                    <?php $no_row++; ?>
									@endforeach
