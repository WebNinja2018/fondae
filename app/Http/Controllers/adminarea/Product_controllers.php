...
class Product_controllers extends Controller
{
	...
	{	
		...
		$result = Category_model::where($info)->orderBy('displayOrder')->get();
	...
	{
		$this->validate($request,array('eventimage' => 'image|required|mimes:jpeg,png,jpg,gif,svg|max:10048',
										
		));

	}	
	else if(isset($request->prodcutImage)){
		$this->validate($request,array('prodcutImage' => 'image|required|mimes:jpeg,png,jpg,gif,svg|max:10048',		
										));
	}
		...
		if ($request->productID!= NULL && $request->productID > 0) 
		{
			...
			$this->validate($request,array('productName'=>'required|unique:product,productName',
											...
											'shortDescription'=>'required',
											//'productExpiredDate'=>'required|date',
											//'prodcutImage' => 'image|required|mimes:jpeg,png,jpg,gif,svg|max:10048'
...
//		}
		...
		$record = array(
						...
						'projectDescription'=>trim($request->projectDescription),
						...
						'featured_page'=>$request->featured_page,
						'staff_pics_page'=>$request->staff_pics_page,
						'popular_page'=>$request->popular_page,
		...
		else
		{
			...
			$recordUrltitle=array('url_title'=>$url_title,'createdby'=>Session::get('admin_user'));
			$query = $product->where('productID', $productID)
    ...
    public function saveMainImage(Request $request)
	{
		...
		if(!empty($image)){
			...
			$destinationPath = public_path('/upload/product/event_image');
			$img = Image::make($image->getRealPath());
			...
			$image->move($destinationPath, $input['imagename']);
			$eventimage=$input['imagename'];
			
		}else
		{
			$eventimage=$request->eventimage_old;
		}
